    ...
    if (!hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            try {
                ...
                $pdo->commit();
                header('Location: index.php?success=' . urlencode('Employee created successfully.'));
                exit;
...
<form method="POST" enctype="multipart/form-data" class="form-card" novalidate>
    ...
    <div class="tab-content" id="tab-ids">
        <p class="tab-note">Record identification document details. Leave blank if not applicable.</p>
        <?php
        $docTypes = [
            ['label' => 'Government Officer ID Card',   'key' => 'doc_officer'],
            ['label' => 'Khmer National Identity Card', 'key' => 'doc_national'],
