...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')) {
        ...
        // Duplicate checks — exclude current employee
        if (!isset($errors['employee_code']) && $values['employee_code'] !== '') {
            $stmt = $pdo->prepare("SELECT id FROM employees WHERE employee_code = ? AND id != ? LIMIT 1");
            $stmt->execute([$values['employee_code'], $id]);
            if ($stmt->fetch()) {
        ...
        if (empty($errors)) {
            try {
                ...
                $errors[] = 'Unable to update employee. Please try again.';
            }
        }
    }
}
...
<div class="page-header">
    ...
    <div class="header-actions">
        <a href="view.php?id=<?= (int)$id ?>" class="btn btn-outline">&larr; Back to Details</a>
    ...
    <div class="form-row">
        <div class="form-group">
            ...
            <input type="number" id="salary" name="salary" min="0" step="0.01"
                   ...
                   value="<?= htmlspecialchars((string)$values['salary'], ENT_QUOTES, 'UTF-8') ?>">
